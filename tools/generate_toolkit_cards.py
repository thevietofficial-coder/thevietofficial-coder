...
def draw_network(draw: ImageDraw.ImageDraw, accent: str) -> None:
    hub = (958, 46)
    satellites = [(1006, 24), (1032, 60), (994, 80)]
    for point in satellites:
        draw.line((hub, point), fill=BORDER, width=1)
        draw.ellipse((point[0] - 3, point[1] - 3, point[0] + 3, point[1] + 3), outline=accent, width=1)
    draw.ellipse((hub[0] - 4, hub[1] - 4, hub[0] + 4, hub[1] + 4), fill=accent)


def render(filename: str, glyph: str, title: str, subtitle: str, tags: tuple[str, ...], accent: str) -> None:
    ...
    draw.text((102, 66), subtitle, font=font(13), fill="#94a3b8")
    draw.line((100, 90, 100 + draw.textbbox((0, 0), subtitle, font=font(13))[2], 90), fill=blend(base_rgb, accent_rgb, 0.6), width=2)

    tag_font = font(16)
    x, y = 32, 116
    tag_fill = blend(base_rgb, accent_rgb, 0.14)
    tag_border = blend(base_rgb, accent_rgb, 0.55)
    for tag in tags:
